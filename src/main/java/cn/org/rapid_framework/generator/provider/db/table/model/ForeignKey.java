...
public class ForeignKey implements java.io.Serializable{
	...
	private void initRelationship() {
		...
		List parentPrimaryKeys    = parentTable.primaryKeyColumns;
		List foreignPrimaryKeys   = foreignTable.primaryKeyColumns;
	...
	public boolean isParentColumnsFromPrimaryKey() {
		boolean isFrom = true;
		List keys = parentTable.primaryKeyColumns;
	...
	private boolean primaryKeyHasColumn(String aColumn) {
		...
		int numKeys = parentTable.primaryKeyColumns.size();
		for (int i=0;i<numKeys;i++) {
			Column sqlCol = (Column)parentTable.primaryKeyColumns.get(i);
